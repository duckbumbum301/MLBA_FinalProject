...
class DashboardPage(QWidget):
    ...
    def setup_ui(self):
        ...
        grid.addWidget(self.card('Mô hình đang hoạt động', 'XGBoost_v3'), 0,0)
        grid.addWidget(self.card('AUC tốt nhất', '0.88'), 0,1)
        grid.addWidget(self.card('Dự báo hôm nay', '124'), 0,2)
        grid.addWidget(self.card('Sức khỏe hệ thống', 'Ổn định'), 0,3)
        root.addLayout(grid)
        perf = QHBoxLayout()
        ...
        dr = QFrame(); dr.setObjectName('Card'); drLay.addWidget(QLabel('Xu hướng tỷ lệ vỡ nợ'))
        perf.addWidget(roc); perf.addWidget(pr); perf.addWidget(dr)
        root.addLayout(perf)
        alerts = QFrame(); alerts.setObjectName('Card'); al = QVBoxLayout(alerts); al.addWidget(QLabel('Cảnh báo')); al.addWidget(QLabel('• Cụm 3 tỷ lệ vỡ nợ 71,8%'))
